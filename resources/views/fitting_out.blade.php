...
@section('contentheader_breadcrumbs')
    <ol class="breadcrumb">
        ...
        <li class="active">Pengembalian FO</li>
    </ol>
@stop
    ...
    <div class="col-md-12">
        ...
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
                ...
                <li><a href="#tab_2" data-toggle="tab">Add Pengembalian FO</a></li>
                @endif
            </ul>
            <div class="tab-content">
                <div class="tab-pane active" id="tab_1">
                <!-- template tabel -->
                    <table id="dg" title="Pengembalian FO" class="easyui-datagrid" style="width:100%;height:100%" toolbar="#toolbar">
                        <!-- kolom -->
                        <thead>
                            <tr>
                                ...
                                <th field="out_number" width="60" sortable="true">No.Keluar</th>
                                <th field="fit_number" width="60" sortable="true">No.FO</th>
                                <th field="out_date" width="40" sortable="true">Date</th>
                                <th field="out_amount" width="50" sortable="true">Amount</th>
                                <th field="out_refno" width="50" sortable="true">RefNo</th>
                                <th field="cashbk_name" width="50" sortable="true">Bank</th>
                                <th field="posting_at" width="40" sortable="true">Tgl Posting</th>
                                <th field="out_post" width="30" sortable="true">Posting</th>
                    ...
                    <div id="toolbar" class="datagrid-toolbar">
                        ...
                        @if(Session::get('role')==1 || in_array(78,Session::get('permissions')))
                        <a href="javascript:void(0)" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" plain="true" onclick="postingInv()" group="" id=""><span class="l-btn-text"><i class="fa fa-check"></i>&nbsp;Posting Selected Pengembalian FO</span>
                        </a>
                        <a href="javascript:void(0)" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" plain="true" onclick="unpostingInv()" group="" id=""><span class="l-btn-text"><i class="fa fa-remove"></i>&nbsp;Unposting Selected Pengembalian FO</span>
                        </a>
                        @endif
                    </div>
                    ...
                    <div id="contractStep1">
                        <form method="POST" id="formPayment">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label>Nomor TTS</label>
                                        <select class="form-control" name="tts_number" id="tts_no">
                                            <option value="PILIH NOMOR TTS">PILIH NOMOR TTS</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label>Pengembalian FO Date</label>
                                        <div class="input-group date">
                                            <div class="input-group-addon">
                                                <i class="fa fa-calendar"></i>
                                            </div>
                                            <input type="text" id="FittingInDate" name="out_date" required="required" class="form-control pull-right datepicker" data-date-format="yyyy-mm-dd">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                            ...
                            <div class="row">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label>Remarks</label>
                                        <textarea name="out_keterangan" class="form-control" rows="4"></textarea>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label>Ref Number</label>
                                        <input type="text" name="out_refno" class="form-control">
                                    </div>
                                </div>
                            </div>
            ...
            <div class="modal-dialog">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Fitting Out Information</h4>
                    </div>
...
<script type="text/javascript">
    var entity = "Pengembalian FO"; // nama si tabel, ditampilin di dialog
    var get_url = "{{route('fittingout.get')}}";
    $('.select2').select2();
    ...
    $('#formPayment').submit(function(e){
        ...
        if(startdate == ''){
          ...
          var i;
          $.post('{{route('fitting.insertfittingout')}}',allFormData, function(result){
    ...
    $(".contrId").change(function(){
        ...
        var url_tts = "{{route('fitting.get_tts')}}";
        var val = $(this).val();

        url = url+"?tenan_id="+val;
        url_tts = url_tts+"?tenan_id="+val;
        $.ajax({
            url: url,
            type: 'GET',
    ...
    function savepayment(){
        ...
        if(ids.length > 0){
            ...
            $.post('{{route('fittingout.posting')}}',data, function(result){
                if(result.success == 1){
    ...
    function unpostingInv(){
        ...
        if(ids.length > 0){
          $.messager.confirm('Confirm','Are you sure you want to unpost this '+ids.length+' Fitting ?',function(r){
              if (r){
                  $('.loadingScreen').show();
                  $.post('{{route('fittingout.unposting')}}',{id:ids}, function(data){
                      alert(data.message);
                      $('.loadingScreen').hide();
                      if(data.success == 1){
                        location.reload();
    ...
    $(document).delegate('.void-confirm','click',function(){
        if(confirm("are you sure you want void this fitting?")){
    ...
    $(document).delegate('.posting-confirm','click',function(){
        if(confirm("are you sure you want posting this fitting ?")){
            var id = $(this).attr('data-id');

            $.post('{{route('fittingout.posting')}}',{id:id}, function(data){
                alert(data.message);
                if(data.success == 1){
                    location.reload();
                }else{
